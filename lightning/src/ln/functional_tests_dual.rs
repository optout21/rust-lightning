...
fn do_v2_channel_establishment_with_rbf(which_tx_to_confirm: u8) {
    ...
    */
    ...
    let expected_tx1_id = "b6f1d021ee8296a9e5062461dde0ca3134b49eebe9efad1a81090604cebca23a";
    ...
    // Create anewfunding inputs, slightly larger (exactly to match the funding output)
    let input_value_rbf = initiator_input_value_satoshis + 124;
    let funding_inputs_rbf = create_dual_funding_utxos_with_prev_txs(&nodes[0], &[input_value_rbf]);
    ...
    let ack_rbf_msg = get_event_msg!(nodes[1], MessageSendEvent::SendTxAckRbf, nodes[0].node.get_our_node_id());
    assert_eq!(nodes[1].node.list_channels().len(), 1);

    // handle ack_rbf on initator side
    let _res = nodes[0].node.handle_tx_ack_rbf(&nodes[1].node.get_our_node_id(), &ack_rbf_msg);

    let tx_add_input_msg = get_event_msg!(&nodes[0], MessageSendEvent::SendTxAddInput, nodes[1].node.get_our_node_id());
    let input_value = tx_add_input_msg.prevtx.as_transaction().output[tx_add_input_msg.prevtx_out as usize].value;
    assert_eq!(input_value.to_sat(), input_value_rbf);
    ...
    let tx_2 = {
        let tx_0 = &nodes[0].tx_broadcaster.txn_broadcasted.lock().unwrap()[1];
        let tx_1 = &nodes[1].tx_broadcaster.txn_broadcasted.lock().unwrap()[1];
        assert_eq!(tx_0, tx_1);
        tx_0.clone()
    };
    let expected_tx2_id = "d07449d51785653c18615af8ed16612ab9a92650a4d9d1b857428403fafc73ec";
    ...
    // Confirm tx
    let (channel_ready, _) = create_chan_between_nodes_with_value_confirm(&nodes[0], &nodes[1], &tx_to_confirm);
