...
/// Depending on how the ID is generated, several varieties are distinguished (but all are stored as 32 bytes): v1 and temporary.
/// The 'v1' channel ID's are generated based on funding tx outpoint (txid&index).
/// Temporary are generated randomly.
/// (Later revocation-point-based 'v2' is a possibility).
