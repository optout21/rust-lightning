	...
	<SP::Target as SignerProvider>::Signer: WriteableEcdsaChannelSigner
{
	...
	/// Based on get_funding_created()
	pub fn get_splice_comm_signed<L: Deref>(&mut self, logger: &L) -> Result<msgs::SpliceCommSigned, ChannelError> where L::Target: Logger 
	{
		if !self.context.is_outbound() {
	...
	/// #SPLICING STEP13 A
	pub fn get_splice_comm_ack<L: Deref>(&mut self, msg: &msgs::SpliceCommSigned, logger: &L) -> Result<msgs::SpliceCommAck, ChannelError>
	where L::Target: Logger 
	{
		if self.context.is_outbound() {
	...
	/// #SPLICING STEP15 I
	pub fn get_splice_signed<L: Deref>(&mut self, msg: &msgs::SpliceCommAck, logger: &L) -> Result<msgs::SpliceSigned, ChannelError> where L::Target: Logger {
		if !self.context.is_outbound() {
	...
	/// #SPLICING STEP17 A
	pub fn get_splice_signed_ack<L: Deref>(&mut self, best_block: BestBlock, signer_provider: &SP,/*splice_transaction: Transaction, splice_txo: OutPoint, splice_prev_funding_input_index: u16, splice_prev_funding_input_value: u64,*/ logger: &L) -> Result<(msgs::SpliceSignedAck, ChannelMonitor<<SP::Target as SignerProvider>::Signer>), ChannelError> where L::Target: Logger {
		if self.context.is_outbound() {
	...
	/// #SPLICING STEP9 A
	pub fn splice_created<L: Deref>(&mut self, msg: &msgs::SpliceCreated, logger: &L) -> Result<msgs::TxComplete, ChannelError>
	where
		L::Target: Logger
	{
		if self.context.is_outbound() {
